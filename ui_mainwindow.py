...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # self.initiate_treeWidget(self.trw_search)  # 初始化treeWidget
        # self.trw_search.header().setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)
        self.category_process_checked = False
        # self.initiate_treeWidget(self.trw_process)  # 初始化treeWidget
        # self.trw_process.header().setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)

        self.category_draw_checked = False
        # self.initiate_treeWidget(self.trw_process_2)  # 初始化treeWidget
        # self.trw_search.header().setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)
...
# ======================================================自定义方法======================================================
    ...
    def check_network(self):
        try:
            self.database.check_network()
        except NETWORK_ERR as ne:
            QMessageBox.warning(self, "警告", ne.__str__())
    ...
    #         if self.database.retry_connect():
    #             messagebox.exec_()
    #     else:
    #         messagebox.close()
    ...
    def initiate_database(self):
        ...
        # result = self.client.query('show measurements;')  # 显示数据库中的表
    ...
    def get_treeWidget_selected(self, trw):
        ...
        #     print("root is pced")
        # elif(root.checkState(0) == Qt.Checked):
        #     print("root is checked")
        # else:
        #     print("not checked")
        return temp
    ...
    def get_trw_selected_iterative(self, leveln_item, category):
        # if(leveln_item.checkState(0) == Qt.PartiallyChecked):
        #     print(leveln_item.text)
    ...
    #mainwindow的tablewidget中各行最后一列的按钮
    ...
    def search(self):
        ...
        if goal != '':
            ...
            # root = self.trw_search.invisibleRootItem().child(0)
            # tmp = []
            for root in roots:
                tmp = []
                if self.is_exist(root, goal):
                    # print("the goal exist")
...
# ======================================================UI组件方法======================================================
    ...
    def on_btn_search_add_clicked(self):
        if self.region:
            ...
            if dialog_1.exec_() == 1:
                ...
                # self.tbw_search_class.add_data_dialog(data)
    ...
    def on_btn_modify_in_result_dialog(self, id):
        ...
        if search_modify_dialog.exec_() == 1:
            ...
            # self.tbw_search_class.delete_data(data[0], data[1])
            ...
            if is_exist:
                self.delete_category(region, old_ctg1, old_ctg2, old_ctg3, old_ctg4, old_ctg5, old_final_name)
                # self.tbw_search_class.delete_data(index, region)
            self.form_category_structure([modified_data])
            # self.tbw_search_class.widget.clearSelection()
            # self.tbw_search_class.add_data_dialog(modified_data)
            # self.tbw_search_class.display()
            # self.database.add_data_dialog(modified_data, self.year)
    ...
    # 查询更新-修正 用户修正数据  
    @QtCore.pyqtSlot()
    # def on_btn_search_correct_clicked(self):
    #     items = self.tbw_search.selectedItems()
    #     if items:
    #         header = ['编码', '地域', '级别1', '级别2', '级别3', '级别4', '级别5', '级别6']
    #         for year in self.year:
    #             header.append(year)
    ...
    def view_detail(self, id):
        ...
        # print(self.tbw_search_class.datas[id].year)
    ...
    def modify(self, id, mode):
        ...
        for i in item.index:
            ...
            # print("data in func modify: ")
            # print(self.database.get_data(query))
            # print("\n\n")
        ...
        if dialog_1.exec_() == 1:
            ...
            # item.setRealName()
            # self.dialog_detail.item = item
            # self.dialog_detail.setTitle(item.realname)
            # self.dialog_detail.table_display()
            # self.tbw_search_class.refresh(id)
            # print("data:", data)
            for i in range(len(data)):
    ...
    # def who(self, id, item):
    #     print("I am No.", id)
    #     print(item)

    # def load_after_add(self, index, region, name):
    #     # print("index in laa: ", index)
    #     # print("region in laa: ", region)
    #     self.contain.clear()
    #     self.contain_tbl.clear()
    #     temp_str = '"编码","地域",'
    #     condition_str = '(' + '"地域"=' + "'%s') and (" % region
    #     condition_str += '"编码"=' + "'%s')" % index
    #     # for cat_index in range(len(cat.index)):
    #     #     if cat_index != len(cat.index) - 1:
    #     #         condition_str += '"编码"=' + "'%s' or " % cat.index[cat_index]
    #     #     else:
    #     #         condition_str += '"编码"=' + "'%s')" % cat.index[cat_index]

    #     for id in range(1, 7):
    #         temp_str += '"级别%d",' % id
    #     for year_index in range(len(self.year)):
    #         if year_index != len(self.year) - 1:
    #             temp_str += '"%s",' % self.year[year_index]
    #         else:
    #             temp_str += '"%s"' % self.year[year_index]
    ...
    #     print(name)
    #     item.year = self.year
    #     self.contain_tbl.append(item)

    #     if self.contain_tbl:
    #         # self.tbw_search_class = TableWidgetSearch(self.tbw_search)
    #         self.tbw_search_class.add_data(self.contain_tbl, self.year)
    #         self.tbw_search_class.display(1)
    #         self.tbw_search_class.widget.clearSelection()
    #         print('\n\n')
    ...
    def query_for_draw(self):
        ...
        # for year_index in range(len(self.selected_year_process)):
        #     if year_index != len(self.selected_year_process) - 1:
        #         temp_str += '"%s",' % self.selected_year_process[year_index]
        #     else:
        #         temp_str += '"%s"' % self.selected_year_process[year_index]
        temp_str += '`年份`'
        query = 'select %s from `Data` where %s' % (temp_str, condition_str)
    ...
    def on_btn_search_complex_clicked(self):
        ...
        # self.findSelected(root.child(0), tmp)
        # self.selected_category_search = self.get_treeWidget_selected(self.trw_search)  # 存放当前在tableWidget中被选中的指标
        # self.selected_region_search = self.contain
        # for srs in self.contain:
        #     print(srs.getName())   #在表中显示的
        # for srs in self.contain:
        #     print(srs.getIndex())  #和原来的selected_category_search一样
        # for cat in self.selected_category_search:
        #     print("cat = ", cat)
        ...
        print('--------------------------------')
        print(self.selected_region_search)
      ...
      #------------------------------checked----------------------------------------------------
        ...
        for region in self.selected_region_search:
            for item in self.contain:
                ...
                for id in range(1, 7):
                    temp_str += '`级别%d`,' % id
                # for year_index in range(len(self.selected_year_search)):
                #     if year_index != len(self.selected_year_search) - 1:
                #         temp_str += '"%s",' % self.selected_year_search[year_index]
                #     else:
                #         temp_str += '"%s"' % self.selected_year_search[year_index]
                temp_str += '`年份`'
                query = 'select %s from Data where %s' % (temp_str, condition_str)
        ...
        for item in self.contain_tbl:
            print("name = ", item.getName())
            print(item.getData())
            print("\n")

        # if not self.contain_tbl:
        #     print("NO-------------")
        # print(self.contain_tbl)
        if self.contain_tbl:
            # self.tbw_search_class = TableWidgetSearch(self.tbw_search)
            ...
            for i in range(self.tbw_search_class.widget.rowCount()):
                self.tbw_search_class.widget.setCellWidget(i, 1, self.buttonForRow(i))
            # for i in range(len(self.contain_tbl)):
            #     print(i)
            #     self.tbw_search_class.detail_for_row[i].clicked.connect(lambda:self.who(i, self.tbw_search_class.detail_for_row[i]))
                # self.tbw_search_class.detail_for_row[i].clicked.connect(lambda:self.who(1, self.tbw_search_class.detail_for_row[1]))
        
            # for i in range(len(self.contain_tbl)):
            #     self.tbw_search_class.detail_for_row[i].clicked.connect(lambda:self.who(i, self.tbw_search_class.detail_for_row[i]))
            #     self.tbw_search_class.modify_for_row[i].clicked.connect(lambda:self.who(i, self.tbw_search_class.modify_for_row[i]))
        # temp_str = '"编码","地域",'
        # condition_str = '('
        # for region_index in range(len(self.selected_region_search)):
        #     if region_index != len(self.selected_region_search) - 1:
        #         condition_str += '"地域"=' + "'%s' or " % self.selected_region_search[region_index]
        #     else:
        #         condition_str += '"地域"=' + "'%s') and (" % self.selected_region_search[region_index]
        # for category_index in range(len(self.selected_category_search)):
        #     if category_index != len(self.selected_category_search)-1:
        #         condition_str += '"编码"=' + "'%s' or " % self.selected_category_search[category_index]
        #     else:
        #         condition_str += '"编码"=' + "'%s')" % self.selected_category_search[category_index]
        # for cat_index in range(1, 7):
        #     temp_str += '"级别%d",' % cat_index
        ...
        #     # for data in datas:
        #     #     print(data)
        #     self.tbw_search_class.add_data(datas, self.selected_year_search)
        #     self.tbw_search_class.display()
        #     self.tbw_search_class.widget.clearSelection()
    ...
    def on_btn_process_forecast_clicked(self):
        ...
        if datas and self.lsw_forecast.selectedItems():
            ...
            years = [int(year) for year in self.selected_year_process]
            values = []
            for i in range(len(datas)):
                temp = np.array([])
                for year in self.selected_year_process:
                    try:
    ...
    def on_btn_draw_plot_2_clicked(self):
        ...
        if self.draw_class.datas and self.lsw_plot_2.selectedItems():
            ...
            # try:
            #     self.draw_class.plot_normal(func)
            # except:
            #     print('绘图函数中存在bug！')
            #     return

            # self.draw_class.display()
            ...
            try:
                ...
                # Qyes = messageBox.addButton(self.tr("打开路径"), QMessageBox.YesRole)
                # Qno = messageBox.addButton(self.tr("好的"), QMessageBox.NoRole)
    ...
    def on_btn_generate_excel_clicked(self):    
        ...
        # self.loading_ui = loading_win(self)
        # self.loading_ui.show()
        # self.loading_ui.btnKill.clicked.connect(self.killThread)
        # print(dir, city, year)
        
        # if dir != "" and city != "" and year != "" and threshold != "":
            # 判断是否具有映射表
            # has_mapping = ['宿迁', '扬州', '泰州', '淮安', '盐城', '连云港', '北京', '无锡', '苏州']
            # if self.template_list.selectedItems():
            #     template_name = self.template_list.selectedItems()[0].text()
            #     if template_name in has_mapping:
            #         auto2.generate_with_mapping(dir, city, year, 'data\\'+template_name+'.xls', template_name, float(threshold))
            #     else:
            #         auto.generate_without_mapping(dir, city, year, 'data\\'+template_name+'.xls', template_name, float(threshold))
            # if self.template_list.selectedItems():
            #     template_name = self.template_list.selectedItems()[0].text()
            #     auto.generate_without_mapping(dir, city, year, 'templates\\'+template_name+'.xls', template_name, float(threshold))

            # # 结束加载框
            # QMessageBox.information(self, 'Finished','年鉴已转化完成！',QMessageBox.Yes, QMessageBox.Yes)
